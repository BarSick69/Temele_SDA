...
void INSERT_LEFT(Ttrenulet* trenuletul,char caracter,FILE* file){
    if(trenuletul->mecanic->prev==trenuletul->locomotiva){
        fprintf(file,"ERROR\n");
        return;
    }
...
void SEARCH(Ttrenulet* trenuletul,char* word,FILE* file){
    int i = 0;
    char letter=word[i];
    Tvagon* tmp=trenuletul->mecanic;
    Tvagon* where;
    while(letter!='\0'){
        ...
        if(tmp->next!=trenuletul->locomotiva){
            tmp=tmp->next;
        }
        else{
            tmp=trenuletul->locomotiva->next;//here differnece
        }
...
void SEARCH_LEFT(Ttrenulet* trenuletul,char* word,FILE* file){
    int i = 0;
    char letter=word[i];
    char cuvant[strlen(word)];
...
void SEARCH_RIGHT(Ttrenulet* trenuletul,char* word,FILE* file){//#|T|EMATEMA
    int i = 0;
    char letter=word[i];//T
    char cuvant[strlen(word) + 1];
    for (int j = 0; j < (int)strlen(word) + 1; j++) {
        cuvant[j] = '\0';
    }
    Tvagon* tmp=trenuletul->mecanic;//T
    while(letter!='\0'){
        if(letter==tmp->caracter){
            cuvant[i]=letter;
            letter=word[++i];//M
...
/*void SEARCH_RIGHT(Ttrenulet* trenuletul, char* word, FILE* file) {
    int i = 0;
    char letter = word[i];
    Tvagon* tmp = trenuletul->mecanic;
    Tvagon* start = tmp; // Store the starting position of the search
    int consecutive_matches = 0; // Track consecutive matches
    ...
    while (letter != '\0') {
        if (letter == tmp->caracter) {
            ...
            i++;
            if (word[i] == '\0') {
                // Word found, move mechanic to the start of the word
                trenuletul->mecanic = start;
                return;
            }
        } else {
            // Reset search if current character doesn't match
            start = tmp->next;
            letter = word[0];
            i = 0;
        ...
        tmp = tmp->next;
        if (tmp == trenuletul->locomotiva) {
            // Word not found, print error and return
            fprintf(file, "ERROR: Word not found.\n");
            return;
        }
        // Mechanic shouldn't pass the same wagon twice
        if (consecutive_matches > 1) {
            // Reset search if consecutive matches exceed 1
            start = tmp->next;
            letter = word[0];
            i = 0;
            consecutive_matches = 0;
...
void SWITCH(listaComenzi* lista){
    comenziNode* current = lista->first;
    comenziNode* prevNode = NULL;
    comenziNode* nextNode = NULL;
    while(current != NULL){
    ...
    lista->first = prevNode;
}


void EXECUTE(listaComenzi* lista,Ttrenulet* trenuletul,FILE* file){
    if(!(strcmp(lista->first->comanda,"MOVE_RIGHT"))){
        ...
        SEARCH_RIGHT(trenuletul,lista->first->param,file);
    }
    else if(!(strcmp(lista->first->comanda,"SEARCH_LEFT"))){
        SEARCH_LEFT(trenuletul,lista->first->param,file);
    }
    else if(!(strcmp(lista->first->comanda,"SEARCH"))){
        SEARCH(trenuletul,lista->first->param,file);
    }
        ...
        INSERT_LEFT(trenuletul,lista->first->param[0],file);
    }
...
void free_lista(listaComenzi* lista){
    if(lista->first!=NULL){
        comenziNode* curr=lista->first;
        comenziNode* tmp;
        while(curr!=NULL){
                    /*printf("lista->last:%p\n",lista->last);
                    printf("lista->first:%p\n",lista->first);
                    printf("lista->first->next:%p\n",lista->first->next);*/
            printf("curr:%p\n",curr);
            tmp=curr;
            curr=curr->next;
            free(tmp);
...
int main(){
    ...
    for(int i = 1;i<n;i++){
        ...
        fscanf(file,"%s",command);
        if(!strcmp(command,"EXECUTE")){  
            EXECUTE(lista,trenuletul,output_file);
            free(tmp);
            continue;
        }
